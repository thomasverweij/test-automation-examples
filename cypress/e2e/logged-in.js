...
// Login step
Given('the user logs in with username from env {string} and password from env {string}', (usernameEnv, passwordEnv) => {
  const username = Cypress.env(usernameEnv);
  const password = Cypress.env(passwordEnv);
  cy.login(username, password);
});

// Logged-in user specific steps
Then('the user info should be visible with name {string}', (name) => {
  cy.get('#authUserLabel').should('be.visible').and('contain.text', name);
});

Then('the auth status should show {string}', (status) => {
  cy.get('#authStatusLabel').should('be.visible').and('contain.text', status);
});
...
Then('the login button should be hidden', () => {
  cy.get('#loginBtn').should('have.class', 'hidden');
});
